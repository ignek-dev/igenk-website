            ...
            <div className="w-full">
              <form action="#" method="POST" onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 gap-x-8 gap-y-9 sm:grid-cols-2 md:gap-y-12 lg:grid-cols-1 xl:grid-cols-2">
                  ...
                  <FormInput label="Email Address" placeholder="example@youremail.com" name="email" type="email" required />
                  <div className="relative z-0 w-full">
                    <label htmlFor="Phone Number" className="text-medium top-1 z-10 origin-[0] -translate-y-8 scale-75 transform text-xl text-white duration-300 lg:text-3xl">
                      Phone Number
                    </label>
                    <PhoneInput
                      international
                ...
                <div className="mt-12">
                  <FormTextArea label="Message" placeholder="Type your message here..." name="message" rows={3} required />
                </div>
                <div className="mt-8.5 sm:mt-16">
                  <button type="submit" disabled={loading} className="rounded-full bg-white px-8.5 py-5 text-lg leading-[1.25rem] font-medium text-black transition-transform hover:scale-105 disabled:cursor-not-allowed disabled:opacity-50">
      ...
      <section className="bg-white py-[64px] text-black">
        <div className="global-container mx-auto w-full px-4 md:px-8">
          <div className="mb-6 grid grid-cols-1 items-end gap-4 sm:mb-14.5 lg:mb-22.5 lg:grid-cols-2 lg:gap-8">
            <div>
              <h2 className="font-semibold text-[#000000]">{officeLocationsSection.title}</h2>
            </div>
          ...
          <div className="flex flex-col gap-7.5">
            {officeLocationsData.map((item) => (
              <div key={item.id} className="flex flex-col lg:flex-row lg:items-start lg:gap-10">
                <div>
                  <Image src={item.image} alt={item.alt} width={853} height={341} className="sm:h[284px] w[710px] h-[284px] w-[361px] w-[853px] lg:h-[341px]" />
                </div>
                <div className="mt-6 flex flex-col lg:mt-0 lg:w-1/2 lg:pl-3.5">
                  ...
                  <div className="mt-5 flex flex-col gap-6 sm:flex-row sm:items-center sm:gap-30 lg:mt-24">
                    <div className="flex min-w-[232px] flex-col gap-1">
                      ...
                      <a href={item.contact.emailHref} className="text-p16 font-medium text-[#151314]">{item.contact.email}</a>
                    </div>
                    <div className="flex min-w-[232px] flex-col gap-1">
                      <p className="text-p18 md:p20 font-medium text-[#766C72]">{item.contact.phoneLabel}</p>
                      <a href={item.contact.phoneHref} className="text-p16 font-medium text-[#151314]">{item.contact.phone}</a>
                    </div>
                  </div>
