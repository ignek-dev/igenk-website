...
export default function LiferayEcommerceDevelopmentContent() {
  ...
  return (
    <main className="pb-0">
        ...
        <div className="global-container mx-auto  pt-[3.281vw] pb-[3.802vw]">
          ...
          <div className="mt-[3.177vw] flex flex-row gap-[1.6vw]">
            {featureTabs.map((label) => (
              <span key={label} className="p20 inline-flex items-center rounded-[2.083vw] border border-[#374151] bg-black px-[1.458vw] py-[0.833vw] text-white shadow-[0_0.208vw_0.521vw_0_rgba(0,151,158,0.25)] transition-colors">
                {label}
              </span>
            ))}
          </div>
      ...
      <section className="bg-[#f7f7f7] text-black">
        <div className="global-container mx-auto flex w-full flex-col gap-[3.021vw] py-[3.333vw]">
          <div className="flex flex-col gap-[1.458vw]">
            ...
            <p className="p18 justify-self-center text-gray-600">{ecommerceOptimization.description}</p>
          </div>
          <div>
            <div className="flex flex-col">
              {ecommerceDevelopment.map((item, index) => (
                <div key={index} className={`flex flex-row items-center border-gray-300 py-[1.458vw] ${index !== 0 ? "border-t" : "pt-0"} ${index !== ecommerceDevelopment.length - 1 ? "border-b" : "pb-0"}`}>
                  <span className="mr-[5.417vw] max-w-[2.76vw] text-[2.083vw] font-medium text-black">{index + 1 < 10 ? `0${index + 1}` : index + 1}</span>
                  <h3 className="text-[1.563vw]! leading-[36px]! font-normal text-black">{item.text}</h3>
                  <p className="ml-auto max-w-[39.198vw] text-left text-[1.042vw] font-normal text-gray-600">{item.description}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="global-container bg-black py-[3.333vw]">
        <div className="mx-auto w-full items-start">
          <div className="flex flex-col gap-[1.458vw]">
            ...
            <p className="p18 justify-self-center text-gray-100!">{ecommerceDevLiferay.description}</p>
          </div>
          <div className="grid grid-cols-1 gap-x-[1.51vw] gap-y-[4.688vw] pt-[4.063vw] md:grid-cols-2">
            {ecommerceDevelopmentUsingLiferay.map((item, index) => (
              <div key={index} className="relative z-[1]  flex flex-col rounded-[16px] border border-[#1F2937] bg-black px-[1.875vw] pt-[1.771vw] pb-[1.771vw]" style={{ boxShadow: "0 0 0 0 rgba(0,0,0,0), 0 0 0 1px rgba(244,244,245,0.10), 0 0 0 0 #FFF" }}>
                <div className="glow-bg absolute inset-0 -z-10"></div>
                <h3 className="z-10 mt-[-3.385vw] w-max rounded-full border border-gray-700 bg-black px-[1.875vw] py-[0.521vw] text-[1.458vw]! font-medium text-white">{item.text}</h3>
        ...
        <section ref={stickyContentRef} className="sticky top-[5.75vw] flex h-auto flex-col overflow-hidden bg-[#f7f7f7] text-black">
          <div className="mx-auto w-full py-[3.333vw]">
            <div className="global-container flex flex-row items-center gap-10 md:grid-cols-2">
              ...
              <p className="p18 text-right w-[46.875vw] leading-relaxed text-[#4B5563]">{ecommerceProcess.description}</p>
            </div>
            <div className="pt-[4.01vw] pl-[10vw]  select-none">
              <motion.div ref={scrollerRef} className="relative flex min-w-max gap-0 will-change-transform" style={{ x: smoothX }}>
                <div className="relative flex min-w-max gap-[4.833vw] pr-[10vw]">
                  ...
                  {steps.map((step, index) => (
                    <div key={index} className="flex w-[24.875vw] flex-col gap-[2.188vw]">
                      <div className="z-10 flex h-[4.167vw] w-[4.167vw] items-center justify-center rounded-full bg-black text-[1.667vw]! text-white">{index + 1}</div>
                      <div className="flex flex-col">
                        <h3 className="h-[4.583vw] w-[21.354vw] text-left text-[1.875vw]! font-medium text-black">{step.title}</h3>
                        <p className="p20 bottom-0 mt-[1.458vw] w-[25.354vw] text-left text-gray-600">{step.description}</p>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <TalkToExpert heading={ecommerceTalkToExpert.heading} description={ecommerceTalkToExpert.description} buttonText={ecommerceTalkToExpert.buttonText} />
      <BlogSection />
    </main>
  )
}
