...
const CalendarPopup = ({ selectedDate, onDateSelect, onClose }: CalendarPopupProps) => {
    ...
    return (
        <div className="absolute top-full left-0 z-20 mt-2 w-full rounded-2xl bg-white p-4 shadow-xl border border-gray-200">
            {/* Year Navigation */}
            <div className="flex items-center justify-between mb-4 px-2">
                <button onClick={() => setDisplayYear(y => y - 1)} className="p-1 rounded-full hover:bg-gray-100 transition-colors">
            ...
            <div className="grid grid-cols-3 gap-2">
                {months.map((month, index) => (
                    <button
                        ...
                        className={`p-2 rounded-lg text-center font-semibold transition-colors text-sm ${
