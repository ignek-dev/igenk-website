  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-6 py-10 text-black shadow-sm">
      ...
      <div className="grid grid-cols-[1fr_auto] items-end gap-4">
        ...
        <button onClick={() => setIsOpen(!isOpen)} className="flex items-center gap-2 text-xl font-medium pb-2 pr-3">
          View Details
          ...
          >
            <div className="border-t border-gray-200 pt-6 text-gray-700">
              ...
              <ul className="mt-2 list-disc pl-5 space-y-1">
                {job.description.responsibilities.map(item => <li key={item}>{item}</li>)}
              </ul>
              
              <h4 className="mt-4 font-bold">Requirements:</h4>
              <ul className="mt-2 list-disc pl-5 space-y-1">
                {job.description.requirements.map(item => <li key={item}>{item}</li>)}
              </ul>
              
              <h4 className="mt-4 font-bold">Preferred Skills:</h4>
              <ul className="mt-2 list-disc pl-5 space-y-1">
                {job.description.preferred.map(item => <li key={item}>{item}</li>)}
              </ul>
            </div>
          </motion.div>
        )}
