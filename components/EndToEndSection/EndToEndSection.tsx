...
export default function EndToEndSection() {
  ...
  const getAnimationClass = (animation: string, delay?: string) => {
    return isInView 
      ? `animate-when-visible ${animation} ${delay || ''}` 
      : initialClass;
  }
  ...
  return (
    ...
    <section ref={sectionRef} className="relative bg-black text-white">
      <div className="mx-auto w-full overflow-x-hidden px-4 py-20 md:px-8 md:py-24 [@media(min-width:1440px)]:px-[150px] [@media(min-width:1920px)]:px-[192px]">
        
        {/* Top Heading */}
        <div className="grid gap-10 md:grid-cols-2 md:gap-12 lg:gap-16">
          <div>
            <h2 className={`${getAnimationClass('animate-slide-top')} text-5xl font-semibold line-height-[60px] letter-spacing-[-0.02em] sm:text-4xl md:text-5xl`} >
              Your End-to-End Digital Transformation Partner
            </h2>
          </div>
          <p className={`${getAnimationClass('animate-slide-top')} max-w-xl text-white/80 text-lg`}>
        ...
        <div className="mt-20 grid gap-12 md:mt-24 md:grid-cols-2 md:gap-14 lg:gap-16">
          ...
          <div className={`${getAnimationClass('animate-slide-right', 'animation-delay-200')} grid gap-10 md:grid-cols-2 md:gap-12`}>
            <div>
              {[
              ...
              ].map((item, index) => <AccordionItem key={index} title={item.title} content={item.content} />)}
            </div>
            <div>
              {[
